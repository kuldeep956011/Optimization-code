...
double Fx(int n, double x[]) {
    double result = 0.0;
    for (int i = 0; i < n - 1; i++) {
        double firstTerm = pow((x[i]-10.0),3);
        double secondTerm = pow((x[i+1]-20.0),3);
...
double Fx(int n, double x[]) {
    double result = 0.0;
    result = -1.0*((pow(sin(2*3.14159*x[0]),3) * sin(2*3.14159*x[1]) ) / (pow(x[0],3) * (x[0]+x[1])));
    printf("Fx = %lf\n",result);
    return result;
}
*/
...
double Gx1(int n, double x[]) {
    double result = 0.0;
    for (int i = 0; i < n - 1; i++) {
        double firstTerm = pow((x[i]-5.0),2);
        double secondTerm = pow((x[i+1]),2);
        result = firstTerm + secondTerm-26.0;
    }
    printf("Gx1 = %lf\n",result);
    if(result>=0.0){
    return 0;
...
double Gx1(int n, double x[]) {
    ...
    for (int i = 0; i < n - 1; i++) {
        double firstTerm = pow((x[i]-5.0),2);
        double secondTerm = pow((x[i+1]-5.0),2);
        result = firstTerm + secondTerm-100.0;
    }
    printf("Gx1 = %lf\n",result);
    if(result>=0.0){
    return 0;
...
double Gx2(int n, double x[]) {
    double result = 0.0;
    for (int i = 0; i < n - 1; i++) {
        double firstTerm = pow((x[i]-6.0),2);
        double secondTerm = pow((x[i+1]-5.0),2);
        result = - firstTerm - secondTerm + 82.81;
    }
    printf("Gx2 = %lf\n",result);
    if(result>=0.0){
    return 0;
...
double Gx1(int n, double x[]) {
    double result = 0.0;
    for (int i = 0; i < n - 1; i++) {
        double firstTerm = -1.0*pow((x[i]),2);
        double secondTerm = x[i+1];
        result = firstTerm + secondTerm - 1.0;
    }
    printf("Gx1 = %lf\n",result);
    if(result>=0.0){
    return 0;
...
double Gx2(int n, double x[]) {
    double result = 0.0;
    for (int i = 0; i < n - 1; i++) {
        double firstTerm = x[i];
        double secondTerm = -1.0*pow((x[i+1]-4.0),2);
        result =  firstTerm + secondTerm - 1.0;
    }
    printf("Gx2 = %lf\n",result);
    if(result>=0.0){
...
double omega(int n, double x[], double r) {
   ...
   if(Gx2(n,x)>=0.0){
    ...
    {
    	result2=100.0+pow(Gx2(n,x),2);
    	printf("m\n");
    }
    result=r*(result1+result2);
	printf("omega = %lf\n",result);
	return result; */
	result = r*(pow(Gx1(n,x),2) + pow(Gx2(n,x),2));
	printf("omega = %lf\n",result);
	return result;
}
...
////////////////////// INITIAL INTERVAL for ALPHA//////////////////////////////
	...
	*/
	...
	double a1=0;
	double b1= 20.0;                                                      //initial interval for function variables as well as ALPHA
	double r=0.01;
